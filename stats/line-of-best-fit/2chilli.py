...
def draw():
  ...
  if coord_list:
    ...
    if len(coord_list) == 2:
      ...
      if score <= (num_of_pts * 2.5):
        push()
        stroke((138, 154, 91))
        graph_line(*get_y(m_rand, c_rand))
        pop()
        prompt_required(score, num_of_pts)
      else:
        push()
        stroke((200, 75, 68))
  ...
  qn_prompt(m_ans, c_ans, m_rand, c_rand)
  drawMousePos()
  push()
  win = False
...
def prompt_required(score, num_of_pts):
  push()
  textSize(15)
  text(f'Can you find the m & c of', 300, 320)
  text('the best fit line?', 300, 300)
  pop()
...
def gen_rand_coords(n, m, c):
  ...
  for i in range(n):
    ...
    y = int(m*x +c + random.randint(-dev,dev))  # This line calculates the y coordinate using the formula m*x + c + random.randint(-5,5) and assigns it to the variable y
    ...
    while (y > 7 or y < -9 or (x,y) in rand_coords) and count < 99:  # This while loop keeps trying to generate a random coordinate until a valid one is found
      ...
      y = int(m*x +c + random.randint(-dev, dev))  # This line recalculates the y coordinate using the new x value
...
def qn_prompt(m_ans, c_ans, m_rand, c_rand):
  push()
  noStroke()
  ...
  if c_ans == c_rand:
    ...
    c_ans = str(new_round(c_ans))
    
  if m_correct and c_correct:
    success()
    celebrate()
  ...
  fill('black')
  text(f"m = {m_ans}, c = {c_ans}", 340, 370)
