...
import math
rand_coords = []
m_rand = random.uniform(-2.5, 2.5)  # This line generates a random float between -2 and 2 and assigns it to the variable m
c_rand = random.uniform(-3, 3)  # This line generates a random float between -3 and 3 and assigns it to the variable c
###STUDENT INPUTS
#$$$
num_of_pts = $$8$$
#$$$
def setup():
  createCanvas (windowWidth, windowHeight)  # This line likely refers to a graphics library that is not included in this code snippet

  gen_rand_coords(num_of_pts,m_rand,c_rand)  # This line calls the gen_rand_coords function with the arguments num_of_pts, m, and c
  #print(rand_coords)
...
def draw():
  ...
  for coord in rand_coords:
    draw_coord(coord, a_blue)
  #graph_line(*get_y(m_rand, c_rand))
  c_correct = find_c((coord1, coord2))
  push()
  stroke('purple')
  ...
  if coord_list:
    ...
    if len(coord_list) == 2:
      ...
      my_score = 0
      for coord in rand_coords:
        dist = distance_to_line_mc(m, c, coord)
        score += dist
        col = gen_col(dist)
        my_dist = distance_to_line_mc(m_rand, c_rand, coord)
        my_score += my_dist
        x = coord[0]
        y = m * x + c
        dotted(coord, (x,y))
        draw_coord(coord, col)
      textSize(15)
      textAlign(RIGHT, CENTER)
      text(f"Your Score: {new_round(max(100 - score, 0))}", 475, 325)
      text(f"My score: {new_round(100 - my_score)}", 475, 305)
      text("Can you beat my score?", 475, 285)
  ...
  qn_prompt(num_of_pts, c_correct)
  drawMousePos()
  push()
  win = False
...
def gen_col(dist):
  thres = 4
  number = max(0, min(dist, thres))
  green = int(255 * (thres - number) / thres)
  red = int(255 * number / thres)
  return (red, green, 0)

def distance_to_line_mc(m, c, point):
  """
  Calculates the distance between a point and a line defined by its slope (m) and y-intercept (c).

  Args:
      m: The slope of the line.
      c: The y-intercept of the line.
      point: A tuple representing the coordinate point (x, y).
  ...
  return distance
def gen_rand_coords(n, m, c):
  for i in range(n):
    x = random.randint(-11, 12)  # This line generates a random integer between -11 and 12 and assigns it to the variable x
    y = int(m*x +c + random.randint(-3,3))  # This line calculates the y coordinate using the formula m*x + c + random.randint(-5,5) and assigns it to the variable y
    count = 0  # This line initializes a variable count to 0
    while (y > 7 or y < -9 or (x,y) in rand_coords) and count < 99:  # This while loop keeps trying to generate a random coordinate until a valid one is found
      x = random.randint(-11, 12)  # This line generates a new random integer between -11 and 12 for x if the previous one was invalid
      y = int(m*x +c + random.randint(-3, 3))  # This line recalculates the y coordinate using the new x value
      count += 1  # This line increments the count variable
      if count == 99:  # This breaks out of the loop if the code cannot generate a valid coordinate after 9999 tries
        break
    rand_coords.append((x,y))  # This line appends the generated coordinate (x,y) to the rand_coords list

def find_c(coord_list):
  m = (coord_list[0][1] - coord_list[1][1])/ (coord_list[0][0] - coord_list[1][0])
  c = coord_list[0][1] - m * coord_list[0][0]
  return c
...
def draw_coord(coord, col):
  push()
  stroke('black')
  fill(col)
  snapped_x = actual_pos_x(coord[0])
  snapped_y = actual_pos_y(coord[1])
  circle(snapped_x, snapped_y, grid_size // 2)
  pop()
...
def qn_prompt(c, c_correct):
  ...
  # c = str(new_round(c))
  textSize(35)
  #colorful_text(f"y= mx+c", 20, 370, 'white', {"c": c_green, 'x' : 'black', 'y': 'black', 'm' : b_yellow}, "")
  fill('white')
  textSize(22)
  textAlign(LEFT, CENTER)
  text("click 2 pts to draw a best fit line!", 20, 370)
  colour = 'white'
  c_correct = round(c_correct, 2)
  if c == c_correct:
    success()
    celebrate()
    colour = green
    ...
    colour = red
  fill('black')
  text("num of pts =", 340, 370)
  text(c, 340 + textWidth("num of pts = "), 370)
