...
def draw():
  ...
  push()
  if len(coord_list) == 2:
    if m == m_correct and c == c_correct:
      success()
      celebrate()
  pop()
  qn_prompt(m_correct, c_correct)
  drawMousePos()
  push()
  win = False
...
def qn_prompt(m, c):
  ...
  c = str(new_round(c))
  textSize(35)
  colorful_text(f"y = {str(m)}x+{str(c)}", 20, 370, 'black', {str(m) : b_yellow, str(c) : c_green}, "")
  fill('white')
  textSize(25)
  fill("black")
  text("Can you draw this line?", 330, 370)
