...
def draw():
  ...
  qn_prompt(m, c, start_coords, end_coords, m_ans)
  drawMousePos()
  push()
  win = False
...
def rise_run(coord_list, m):
  ...
  fill(r, g, b, 200)
  noStroke()
  rect(x - 5, y - grid_size * 2, textWidth('slope = rise/run') + 10, grid_size * 2 + 25, 25)
  fill(c_green)
  text('slope ', x, y)
  fill('black')
  text('= rise/run', x + textWidth('slope '), y)

  text(f'= ?', x + textWidth('slope '), y - grid_size)
...
def qn_prompt(m, c, start, end, m_ans):
  push()
  noStroke()
  ...
  textSize(35)
  colorful_text(f"y = mx+c", 20, 370, 'black', {'m' : c_green, 'c' : a_blue, str(m_ans): colour}, "")
  fill('white')
  textSize(23)
  fill("black")
  text("click 2 points on line!", 280, 370)
  textSize(30)
  colorful_text(f"m={str(m_ans)}", 430, 370, 'black', {'m' : c_green, 'c' : a_blue, str(m_ans): colour}, "")
