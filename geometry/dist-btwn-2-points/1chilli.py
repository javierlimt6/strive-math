...
is_pressed = False
grid_size = 25
zero_pt = (250,200) # both x, y coords must be in multiples of the grid_size, zero_pt cannot be (0,0)
...
def correct_resp(start, end):
  ...
  colorful_text(f'a = {max(start[0], end[0])}-{brkt(min(start[0], end[0]))} ={str(x_dist)}', 325, 370, "black", {"a": a_blue, str(x_dist): "red"}, "")
  colorful_text(f'b = {max(start[1], end[1])}-{brkt(min(start[1], end[1]))} ={str(y_dist)}', 325, 340, "black", {"b": b_yellow, str(y_dist):"blue"}, "")
  colorful_text(f'c = ?', 325, 310, "black", {'c': c_green, str(y_dist):"blue"}, "")
  pop()
def mouseClicked():
  redraw()
